...
macOS (2026-09-09): first build, tests and launch verified on the M3 Max MacBook; the run log, two fixes (bundled-MoltenVK pin in `src/macos/src/mpv_host.rs`, boot-time sync mpv reads moved off the main thread via `mpv_read_off_main` in `src/jfn_rust/src/app.rs`) and the open items (login/playback/video-mode checks, an intermittent CEF GPU-thread segfault on shutdown, the never-run GitHub macOS workflows) are in `docs/macos-plan.md` under "Verified on the MacBook". `dev/macos/setup.sh` now upgrades stale formulas from its own list; build with `/opt/homebrew/bin` first on PATH.
