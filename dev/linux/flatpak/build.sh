...
MANIFEST="${SCRIPT_DIR}/io.github.thehalfrican.Astrofin.yml"
APP_ID="io.github.thehalfrican.Astrofin"
VERSION="$(cargo run --quiet --manifest-path "${REPO_ROOT}/src/xtask/Cargo.toml" -- version)"
DATE="$(date -u +%Y-%m-%d)"
ARCH="$(uname -m)"
BUNDLE_NAME="Astrofin-${VERSION}-linux-${ARCH}.flatpak"
...
python3 "${SCRIPT_DIR}/generate_metainfo.py" \
    --template "${REPO_ROOT}/resources/linux/io.github.thehalfrican.Astrofin.metainfo.xml" \
